...
export class RecommendationService {
  ...
  constructor(aiServiceUrl: string = process.env.AI_SERVICE_URL || 'http://localhost:3007') {
  ...
  private async getAIRecommendations(
    ...
    try {
      ...
      const response = await fetch(`${this.aiServiceUrl}/api/ai/advising/recommend`, {
        method: 'POST',
   ...
   */
  private getFallbackRecommendations(degreeProgress: any): string[] {
  ...
  private async buildCourseRecommendations(
    ...
    for (let i = 0; i < courseCodes.length; i++) {
      ...
      const sections = availableSections.filter(s => s.courseId === course.id)

      // Check which sections fit the schedule
      const compatibleSections = sections.filter(section => {
        const sectionSlots = sectionToTimeSlots(section.days, section.startTime, section.endTime)
        ...
        if (athleticSchedule) {
          for (const event of athleticSchedule.events) {
            ...
            if (hasTimeSlotOverlap(sectionSlots, eventSlots) && event.isMandatory) {
              return false
            }
          }
        }
      ...
      const meetsPrerequisites = !course.prerequisites || course.prerequisites.length === 0

      const fitsSchedule = compatibleSections.length > 0

      // Calculate priority (higher is better)
      const priority = this.calculateCoursePriority(course, i, fitsSchedule, meetsPrerequisites)
  ...
  private generateCourseReasoning(
    ...
    if (fitsSchedule && sectionCount > 0) {
      reasons.push(`${sectionCount} section(s) available that fit your schedule`)
    } else if (!fitsSchedule) {
      reasons.push('No sections currently fit your schedule - consider adjusting')
    }
  ...
  private generateRecommendationWarnings(
    ...
    const currentCredits = currentSchedule.reduce((sum, s) => sum + (s.course?.credits || 0), 0)
  ...
  private generateReasoning(
    ...
    parts.push(`we recommend the following courses for ${request.term} ${request.academicYear}. `)
    ...
    if (incompleteCats.length > 0) {
      parts.push(
        `Focus on completing requirements in: ${incompleteCats.map((r: any) => r.category).join(', ')}. `
      )
    }
