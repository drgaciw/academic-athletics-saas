...
export class ConflictDetectorService {
  ...
  async detectConflicts(
    ...
    if (athleticSchedule) {
      const athleticConflicts = this.detectAthleticConflicts(
        sections,
        athleticSchedule,
      );
    ...
    if (prerequisiteData) {
      const prereqConflicts = this.detectPrerequisiteViolations(
        sections,
        prerequisiteData,
      );
  ...
  private buildConflictGraph(sections: CourseSectionInfo[]): ConflictGraph {
    ...
    for (const section of sections) {
      const timeSlots = sectionToTimeSlots(
        section.days,
        section.startTime,
        section.endTime,
      );
    ...
    for (let i = 0; i < sectionIds.length; i++) {
      for (let j = i + 1; j < sectionIds.length; j++) {
        ...
        if (hasTimeSlotOverlap(node1.timeSlots, node2.timeSlots)) {
          ...
          edges
            .get(node2.id)!
            .push({ ...edge, fromNodeId: node2.id, toNodeId: node1.id });
        }
      }
    }
      ...
      for (let j = i + 1; j < sections.length; j++) {
        ...
        if (overlaps.length > 0) {
          const overlapDetails = overlaps
            .map(
              (o) =>
                `${formatTimeSlot(o.slot1)} overlaps with ${formatTimeSlot(o.slot2)}`,
            )
            .join("; ");
   ...
   * Detect corequisite violations
   */
  private detectCorequisiteViolations(
    sections: CourseSectionInfo[],
  ): Conflict[] {
    const conflicts: Conflict[] = [];
    const courseIds = new Set(
      sections.map((s) => s.course?.id).filter(Boolean),
    );
  ...
  private generateWarnings(sections: CourseSectionInfo[]): string[] {
    ...
    if (totalCredits > 18) {
      warnings.push(
        `Heavy course load: ${totalCredits} credit hours (standard maximum is 18)`,
      );
    }
