...
app.post("/validate-schedule", async (c) => {
  try {
    ...
    const prerequisiteData = validatedData.checkPrerequisites
      ? undefined
      : undefined;

    // Calculate total credits
    const totalCredits = sections.reduce(
      (sum: number, s: any) => sum + (s.course?.credits || 0),
      0,
    );
    ...
    if (validatedData.checkCreditLimit) {
      if (totalCredits < CREDIT_HOUR_LIMITS.MIN_FULL_TIME) {
        ...
        suggestions.push(
          "Add more courses to meet full-time enrollment requirements",
        );
        ...
        if (totalCredits > CREDIT_HOUR_LIMITS.MAX_WITH_APPROVAL) {
          isValid = false;
          suggestions.push(
            "Reduce course load or seek approval for credit hour overload",
          );
        } else {
    ...
    if (isValid && conflicts.length === 0) {
      ...
      suggestions.push(
        "Schedule has minor issues but is enrollable. Consider addressing warnings.",
      );
    }
